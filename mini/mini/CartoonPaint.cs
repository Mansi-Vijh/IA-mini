...
namespace mini
{
    ...
    public partial class CartoonPaint : Form
    {
        ...
        List<Image<Gray, byte>> out_gray_arr = new List<Image<Gray, byte>>();
        Capture _capture;
        Image<Bgr, byte> img;
        Image<Bgr, byte> painted;
        //Image<Gray, byte> img_s;
        //Image<Bgr, byte> img_sketch_original;
        ...
        private void sketch()
        {
            for (int a = 0; a < arr.Count(); a++)
            {
                Image<Gray, byte> img_s = new Image<Gray, byte>(arr[a].Bitmap);
              //  img_s = arr[a].Convert<Gray, Byte>();



                int[,] sobel_x = new int[3, 3] { { -1, 0, 1 }, { -2, 0, 2 }, { -1, 0, 1 } };
                int[,] sobel_y = new int[3, 3] { { 1, 2, 1 }, { 0, 0, 0 }, { -1, -2, -1 } };
                
                Image<Gray, byte> res1 = new Image<Gray, byte>(img.Size);
                //   double[,] part = new double[3, 3];
                double gx = 0, gy = 0, g;
                int[,] temp = new int[img_s.Width + 2, img_s.Height + 2];


                for (int i = 0; i < (img_s.Height + 2); ++i)
                    for (int j = 0; j < (img_s.Width + 2); ++j)
                    {
                        temp[i, j] = 0;

                    }
                for (int i = 0; i < img_s.Height; i++)
                    for (int j = 0; j < img_s.Width; j++)
                    {
                        temp[i + 1, j + 1] = img_s.Data[i, j, 0];
                    }
                for (int i = 0; i < img_s.Height; i++)
                    for (int j = 0; j < img_s.Width; j++)
                    {
                        ...
                        for (int m = 0; m < 3; m++)
                            for (int n = 0; n < 3; n++)
                            {
                                gx = gx + (sobel_x[m, n] * temp[i + m, j + n]) / 8;
                                gy = gy + (sobel_y[m, n] * temp[i + m, j + n]) / 8;
                            }
                        g = Math.Sqrt(gx * gx + gy * gy);
                        res1.Data[i, j, 0] = (byte)g;
                    }
                out_gray_arr.Add( res1);
        ...
        private void btn_sketch_Click(object sender, EventArgs e)
        {
            ...
            sketch();
            //join();
            //result.Image = out_gray_arr[2];
            for (int a = 0; a < out_gray_arr.Count(); a++)
            {
                string OutFileLocation = "D:\\IIITD\\Sem5\\IA\\IA mini";
                try
                {

                    out_gray_arr[a].Save(OutFileLocation + "\\" + "op" + DateTime.Now.Second + "-" + DateTime.Now.Millisecond + ".jpg");
